...
{\LARGE\bfseries 任务一实训论文}\\[1.5cm]
{\Large\bfseries 基于传统机器学习的白葡萄酒质量分类研究}\\[2cm]
{\large
...
\par 葡萄酒质量预测是结构化数据分类中的典型问题。本文以 UCI Wine Quality 数据集中的白葡萄酒子集为
...
\par\vspace{1ex}
\noindent\textbf{关键词：}机器学习；白葡萄酒质量；支持向量机；随机森林；有序建模；多分类
...
\subsection{研究内容与分工}
本文围绕白葡萄酒质量三分类任务开展实验研究，主要工作包括：数据集加载与查看、质量评分三分类标签构造、
特征标准化与分层划分、四种传统机器学习模型训练、测试集指标评价、混淆矩阵与特征重要性分析，以及基于
误差结构的有序建模补充实验。小组成员分工如表~\ref{tab:team} 所示。
...
\begin{table}[H]
  ...
  \begin{tabular}{ll}
    ...
    文件 & 功能 \\
    \midrule
    wine\_quality.py & 数据下载、EDA、预处理、四模型训练与评估 \\
    metrics.csv & 四种模型基础评估指标 \\
    metrics\_extra.csv & 平衡准确率、Kappa、MCC、ROC-AUC 等进阶指标 \\
    ordinal\_improvement.csv & 有序回归分级对照结果 \\
    model\_compare.png & 模型指标对比图 \\
    cm\_随机森林.png & 随机森林混淆矩阵 \\
    rf\_feature\_importance.png & 随机森林特征重要性图 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\end{quote}
脚本会自动下载数据集，完成探索性分析、标签构造、模型训练、网格搜索、测试集评估和图表输出，结果保存至
