...
难度来源；极端类别不平衡会使准确率严重失真，须以宏平均 F1 或平衡准确率衡量。针对表现欠佳的数据集，
为贷款违约补充申请时类别特征、为极不平衡的臭氧数据加入 SMOTE 过采样后，宏平均 F1 均获得明显提升，
说明这些“效果不好”更多源于处理不充分而非模型本身。

\par\vspace{1ex}
\noindent\textbf{关键词：}机器学习；结构化数据分类；模型对照；类别不平衡；SMOTE；跨数据集
...
关系；若逻辑回归表现明显偏低，通常意味着需要非线性模型捕捉更复杂的特征组合。
...
\begin{table}[H]
  ...
  \begin{tabular}{lccccc}
    ...
    贷款违约 Lending Club & 0.5698 & 0.5380 & 0.5343 & \textbf{0.5843} & 逻辑回归 \\
    干豆 Dry Bean & \textbf{0.9375} & 0.9104 & 0.9333 & 0.9300 & SVM \\
    臭氧 Ozone & 0.5805 & \textbf{0.5806} & 0.4920 & 0.5516 & 决策树 \\
    \bottomrule
  \end{tabular}}
\end{table}
...
跨数据集对照还暴露了两类容易被准确率掩盖的问题，如图~\ref{fig:tab-imb} 所示。
...
\begin{table}[H]
  ...
  \begin{tabular}{llcl}
    ...
    \multirow{3}{*}{贷款违约}
      ...
      & 梯度提升 GBDT（37 维） & 0.5739 & 更强模型亦无突破 \\
    \midrule
    \multirow{3}{*}{臭氧}
      & 基线（class\_weight 平衡） & 0.5806 & --- \\
      & SMOTE 过采样 & 0.5587 & 不升反降：高维且少数类样本过少，插值成噪声 \\
      & 梯度提升 GBDT & 0.5778 & 更强模型亦无突破 \\
    \bottomrule
  \end{tabular}}
\end{table}
...
而非有效样本。其三，进一步换用更强的梯度提升模型（GBDT），两个数据集的宏平均 F1 仍分别停在 0.574 和
0.578，未能突破四个基础模型约 0.58 的上限。

由此可以确认：这两个数据集的“效果不好”\textbf{主要源于数据本质（弱判别信号、极端不平衡叠加高维小样本）
而非处理不充分}——补特征、重采样、换更强模型三类手段均无法突破其性能上限。真正起作用的反而是“模型与数据
的匹配”：在臭氧上避开会崩溃的随机森林、改用决策树或支持向量机并配合类别权重，已是该任务的较优选择。
...
宏平均 F1 或平衡准确率评价。第四，当数据信号本身较弱时，性能瓶颈在数据而非模型——经补特征、重采样、
更换更强模型三类手段验证，贷款违约与臭氧的性能上限均无法突破，证明其“效果不好”源于数据本质而非处理
不充分。综合而言，\textbf{传统机器学习的性能由“数据特点”与“模型—数据匹配”共同决定；改进前须先诊断
瓶颈来源，集成树模型通常是稳健默认，但面对极端不平衡或弱信号数据，盲目套用标准技巧未必有效}。
...
\end{quote}
第一条复现白葡萄酒主实验（自动下载数据），后两条复现五数据集对照与改进尝试。其余四个数据集需预先放入
\texttt{task1\_ml\_wine/data/}。所有结果保存至 \texttt{task1\_ml\_wine/outputs/}。
...
\subsection{不足与展望}
本文仍存在一些不足。首先，实验只比较了四种传统模型，后续可加入 XGBoost、LightGBM 等梯度提升方法；
其次，类别不平衡只通过类别权重和分层划分进行处理，后续可系统比较 SMOTE 等重采样策略；最后，有序回归
分级实验仍较基础，后续可进一步使用专门的有序分类模型和多随机种子重复实验验证结论稳定性。
