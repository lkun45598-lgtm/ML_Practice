...
% —— 目录格式：标题黑体4号居中、字间空4字距；各级条目宋体小4号、小黑点连接、页码右对齐 ——
\usepackage{tocloft}
\renewcommand{\contentsname}{目\hspace{4\ccwd}录}
\renewcommand{\cfttoctitlefont}{\hspace*{\fill}\heiti\zihao{4}}
...
\newpage
\pagenumbering{Roman}   % 正文之前（摘要/Abstract/目录）用 I、II 等罗马数字
\setcounter{page}{1}
...
\tableofcontents
\clearpage
\pagenumbering{arabic}  % 正文页码从 1 重新标起

% ===================== 第一章 引言 =====================
\section{引言}

\subsection{研究背景与意义}
图像分类是计算机视觉中的基础任务，也是深度学习方法最早取得显著突破的方向之一。卷积神经网络（Convolutional Neural Network，CNN）通过局部
...
AlexNet 在 ImageNet 图像分类任务中的成功推动了深度卷积网络的快速发展。其结构包含多层卷积、池化、修正线性单元（Rectified Linear Unit，ReLU）
...
完整流程；其二，通过组件消融、复杂度对比、梯度加权类激活映射（Gradient-weighted Class Activation Mapping，Grad-CAM）可视化以及跨数据集的架构对照，可以从“模型”而非
...
Srivastava 等（2014）系统研究了 Dropout 对过拟合的抑制作用。Ioffe 和 Szegedy（2015）提出批归一化（Batch Normalization，BN），
...
AlexNet 由五个卷积层和三个全连接层组成。原始 AlexNet 使用局部响应归一化（Local Response Normalization，LRN）和 Dropout 等组件。本文
...
的 co-adaptation，缓解大规模全连接层带来的过拟合风险。优化方面，本文采用带动量的随机梯度下降（Stochastic Gradient Descent，SGD）。动量项能够累积历史
...
本身的特点决定，可归纳为三个因素。四个数据集由易到难的难度阶梯及其代表样例如图~\ref{fig:datasets} 所示。

\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\textwidth]{fig_datasets.png}
  \caption{四个数据集的难度阶梯：从标准灰度图到复杂彩色图像，数据量递减、难度递增}
  \label{fig:datasets}
\end{figure}
...
（如图~\ref{fig:kernel} 所示）。换言之，性能的提升并非来自更多参数，而是来自“更小的核、更深的层和更强的
非线性”这一结构选择，且越困难的数据集收益越大。

\begin{figure}[H]
  \centering
  \includegraphics[width=0.92\textwidth]{fig_kernel_compare.png}
  \caption{大卷积核与小卷积核堆叠的对比：相同感受野下小核更省参、非线性更强}
  \label{fig:kernel}
\end{figure}
...
而小型 ResNet 用全局平均池化（Global Average Pooling，GAP）取代了庞大的全连接头，几乎 100\% 的参数都用在卷积提特征上
（两种分类头的对比见图~\ref{fig:head}）。这正是 ResNet 以极少参数取胜的根本原因——AlexNet 多出的参数
大多用在了易过拟合的全连接头上，并没有用在刀刃上。具体而言，小型 ResNet 的优势来自三处架构设计：其一，
全部使用 $3\times3$ 小核堆叠，与上一节的受控对照结论一致；其二，引入残差连接，为梯度提供恒等捷径，使网络
可以堆叠到十余层卷积而不发生退化，从而学到更丰富的层级特征（残差块结构见图~\ref{fig:resblock}）；其三，
用全局平均池化替代大型全连接层，将每个通道压缩为一个值后直接分类，参数极少、自带平移不变性，过拟合风险
也显著降低。小型 ResNet 的整体结构如图~\ref{fig:resnet} 所示。

\begin{figure}[H]
  \centering
  \includegraphics[width=0.66\textwidth]{fig_resblock.png}
  \caption{残差块（BasicBlock）结构：捷径使输出为 $F(x)+x$，缓解深层退化}
  \label{fig:resblock}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\textwidth]{fig_resnet_arch.png}
  \caption{小型 ResNet 整体结构：Stem + 三个 Stage（各含 2 个残差块）+ 全局平均池化}
  \label{fig:resnet}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.92\textwidth]{fig_head_compare.png}
  \caption{巨型全连接头与全局平均池化头的对比：后者参数少四个数量级、过拟合更轻}
  \label{fig:head}
\end{figure}
