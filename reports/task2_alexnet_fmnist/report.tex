...
\texttt{--small-kernel}、\texttt{--loss} 和 \texttt{--epochs} 参数组合运行。跨数据集对照可执行
\texttt{run\_cross\_parallel.sh}（三模型四数据集）、\texttt{run\_scnn\_fair.sh}（公平基线）和
\texttt{run\_kernel\_ablation.sh}（大核 vs 小核），再用 \texttt{cross\_dataset\_summary.py} 聚合。
其中 Fashion-MNIST 会自动下载，三个彩色数据集需预先下载并解压至 \texttt{task2\_alexnet\_fmnist/data/} 下
对应目录。
