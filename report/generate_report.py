...
def abstract(doc):
    ...
    para(doc,
         ...
         "完成十分类任务，基线测试准确率 91.55%。在此基础上，本文进一步提出“诊断—改进—验证”的误差驱动"
         "改进方法论：项目一据“邻级混淆”改用有序回归分级，将严重误判减少 43%；项目二据误差分析与可解释性"
         "证据，采用 BatchNorm、数据增强与更长训练，将准确率提升至 94.38%。两个项目完整覆盖了数据加载、"
...
def chapter_innovation(doc):
    ...
    doc.add_heading("5.2 项目二改进：BatchNorm + 数据增强 + 更长训练", level=2)
    para(doc, "诊断：综合第 4 章对照与误差分析，基线 AlexNet 有三处可改进点——(1) LRN 被证明无益；"
              "(2) 仅训练 15 轮，模型欠拟合（这也是早期数据增强不升反降的原因：增强加大优化难度而轮数不足）；"
              "(3) 误差集中于视觉相似的上装，需更强的特征表达。改进：采用现代训练配方——用 BatchNorm 替代 LRN、"
              "引入数据增强（随机翻转+旋转）、并将训练延长至 40 轮配合余弦退火学习率。验证：各阶段结果见"
              "表 5-2 与图 5-2。")
    add_table(doc, ["配置", "测试准确率", "宏平均F1", "衬衫类F1"],
              [["基线（15轮，LRN）", "0.9155", "0.9153", "0.77"],
               ["长训练（40轮，LRN）", "0.9314", "0.9311", "0.79"],
               ["+ BatchNorm（40轮）", "0.9432", "0.9429", "0.83"],
               ["+ BatchNorm + 数据增强（40轮）", "0.9438", "0.9437", "0.83"]],
              caption="表 5-2 AlexNet 渐进式改进结果")
    add_image(doc, os.path.join(T2, "improve_compare.png"), 4.8,
              "图 5-2 AlexNet 渐进式改进：BatchNorm/增强/更长训练显著提升精度")
    para(doc, "改进效果显著：测试准确率由基线 91.55% 提升至 94.38%（+2.83 个百分点），最难的“衬衫”类 F1 由 "
              "0.77 升至 0.83。值得注意的是，延长训练后数据增强转为正收益（0.9432→0.9438），印证了“早期增强"
              "失效是因轮数不足而非增强本身无效”；BatchNorm 带来的最大单步提升，也再次说明该任务瓶颈在于特征表达。")
    ...
    para(doc, "本章通过“诊断—改进—验证”闭环在两个项目上均取得实质成果：项目一用有序回归将严重误判由 7 降至 4；"
              "项目二通过 BatchNorm + 数据增强 + 更长训练，将测试准确率由 91.55% 提升至 94.38%、衬衫类 F1 由 "
              "0.77 升至 0.83，并借助 Focal Loss 的否证对照与 Grad-CAM 可解释性，论证了“瓶颈在特征表达”的因果"
              "结论。相比“仅报告精度”的常规做法，这一方法论是本报告最具价值的创新点。")
    doc.add_page_break()
