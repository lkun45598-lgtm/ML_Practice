...
def chapter4(doc):
    ...
    para(doc, "训练与验证过程的损失、准确率曲线见图 4-2。可见训练初期损失迅速下降、准确率快速提升；随着"
    ...
    para(doc, "上节主模型的若干设计（去除 LRN、是否数据增强、网络容量）并非凭经验拍定，而是先通过一组"
              ...
              "短预算，并非主模型的训练设置（主模型为 40 轮）。结果见表 4-3 与图 4-4。其中 SimpleCNN 为仅含 "
...
def chapter_innovation(doc):
    ...
    add_image(doc, os.path.join(T2, "improve_compare.png"), 4.8,
