...
# 结果同样写 outputs/exp_x_*.json，由 analysis/cross_dataset_summary.py 聚合。
...
alex() {
  ...
  CUDA_VISIBLE_DEVICES=$gpu $PY -m task2_alexnet_fmnist.experiments --tag "$tag" --device cuda --amp \
    ...
    --epochs $AEP --batch-size 128 --seed 0 --save-history "task2_alexnet_fmnist/outputs/hist_$tag.json" \
